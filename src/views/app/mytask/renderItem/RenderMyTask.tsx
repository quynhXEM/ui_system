...
interface props {
  ...
  onClickDetailTask: (id: string) => void
  myID: string | undefined
  item: TaskType
}

const RenderMyTask = (props: props) => {
  ...
  const handlShowDetailTask = () => {
    props.onClickDetailTask(props.item.id)
  ...
  return (
    <div>
      <Card className='flex items-center justify-between p-2.5 my-3 shadow-sm cursor-pointer' >
        <div className='w-full row flex' onClick={handlShowDetailTask}>
          <Avatar
            alt='John Doe'
